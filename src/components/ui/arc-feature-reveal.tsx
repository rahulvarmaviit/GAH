...
export function ArcFeatureReveal() {
    ...
    useMotionValueEvent(scrollYProgress, "change", (latest) => {
        // Map scroll progress (0 to 1) to feature index (0 to features.length - 1)
        const totalFeatures = features.length;
        // Divide the scroll space into equal segments for each feature
        // e.g., 4 features: 0-0.25 -> 0, 0.25-0.5 -> 1, 0.5-0.75 -> 2, 0.75-1.0 -> 3
        ...
        // Auto-expand right side menus after scrolling past WFX (last feature)
        // When scroll progress > 80%, show the right menus
        if (latest > 0.8) {
            setIsExpanded(true);
        } else {
            setIsExpanded(false);
